...
CREATE OR REPLACE FUNCTION prod_detail(product_id INTEGER, branch INTEGER)
    ...
    FROM (SELECT s.prod_id, prod_name, display_location, stock_location, exp_date, lot_number, s.amount FROM stocks s
              INNER JOIN orders o ON lot_number = trans_id
              WHERE s.prod_id = product_id AND s.amount > 0 AND s.branch_id = branch) as s
