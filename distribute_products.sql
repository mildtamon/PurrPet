...
create or replace procedure distribute(to_branch INTEGER, product_id INTEGER, dist_amount INTEGER, lot varchar) as
   $$
    BEGIN
        ...
        PERFORM 1
        FROM stocks
        WHERE prod_id = product_id AND branch_id = to_branch AND lot_number = lot;
        ...
        UPDATE stocks
        SET amount = amount - dist_amount
        WHERE prod_id = product_id AND branch_id = 1 AND lot_number = lot;
